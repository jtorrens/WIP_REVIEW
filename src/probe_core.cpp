...
PointI computeTextOrigin(RectI outputBounds, int maskWidth, int maskHeight,
  ...
  const int left = outputBounds.x1 +
      static_cast<int>(std::lround(options.paddingLeft * width));
  const int right = outputBounds.x2 -
      static_cast<int>(std::lround(options.paddingRight * width));
